...
def process_image(img_str):
    ...
    return json.dumps(
        {'title': result['title'], 'location': result['location'], 'descr': result['descr'][0], 'url': result['url'],
         'wikipedia': result['wikipedia'], 'image': finded_img_str})
...
def process_text(text):
    ...
    finded_img_str = base64.b64encode(buffered.getvalue()).decode('utf-8')
    return json.dumps(
        {'title': result['title'], 'location': result['location'], 'descr': result['descr'], 'url': result['url'],
         'wikipedia': result['wikipedia'], 'image': finded_img_str})
...
def process_museum_image(img_str):
    ...
    for i in data_museum.keys():
        if o == n:
            ...
            break
        n += 1
        ''' o = cos(image_features, torch.Tensor(data_museum[i]['image_vector']).to("cpu"))
        if o > n:
           res = i
           n = o '''
    print(o)
    ...
    finded_img_str = base64.b64encode(buffered.getvalue()).decode('utf-8')
    return json.dumps(
        {'title': result['title'], 'author': result['author'], 'descr': result['descr'], 'image': finded_img_str})
