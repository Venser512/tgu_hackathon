...
В боте есть меню, в котором представлены 4 команды:
/sign - поиск достопримечательности по фото
/paint - поиск картины по фото
/sign_text - поиск достопримечательности по тексту
/paint_text - поиск картины по тексту

После выбора пунктов меню (/paint и /sign) можно отправить фото и получить описание ответным сообщением.
После выбора пунктов меню (/paint_text и /sign_text) можно отправить текст и получить фото ответным сообщением.


### Приложение (Дообучение VGG16)
(Ноутбук model_copy.ipynb)
1. Расширение датасета (аугментация):

Первая аугментация:

	Загружает CSV-файл (description2.csv), содержащий информацию об изображениях.
	Для каждого изображения:
        Загружает изображение.
	Применяет случайные аугментации (размытие, изменение яркости и контраста, небольшие сдвиги и повороты).
	Сохраняет аугментированное изображение в указанную директорию (./data/).
	Добавляет новую строку в DataFrame с информацией о новом аугментированном изображении.
	Объединяет исходные данные и аугментированные данные в новый CSV-файл (augmented_description2.csv).

Вторая аугментация:

	Загружает уже аугментированный CSV-файл (augmented_description2.csv).
	Выполняет аналогичные действия, как и первая аугментация, но с немного другими параметрами аугментации.
	Сохраняет результат в augmented_description2_extended.csv.

2. Обучение и сохранение модели в файл и извлечение признаков из изображений:

Загрузка данных: Загружает расширенный CSV-файл (augmented_description2_extended.csv).
Предобработка изображений: Загружает изображения из CSV, изменяет их размер до 224x224 пикселей и нормализует значения пикселей.

Создание модели VGG16 для извлечения признаков:
	Загружает предварительно обученную модель VGG16 (без классификатора).
	Замораживает веса слоев VGG16.
	Добавляет свой полносвязный слой поверх VGG16.
	Создает модель для извлечения признаков.
Сохранение модели: Сохраняет обученную модель в файл (feature_extraction_model.h5).
Извлечение признаков из изображений: Использует модель для извлечения признаков из всех изображений в датасете.
Функция find_closest_image: Определяет функцию, которая находит наиболее похожее изображение на основе косинусного расстояния.
Пример использования: Использует функцию find_closest_image для демонстрации работы модели.

3. Сохранение признаков в файл:

Выполняет шаги из предыдущего пункта (Загрузка данных, Предобработка, Создание модели, Извлечение признаков).
Сохранение признаков: Сохраняет извлеченные признаки в файл image_features.pkl с использованием библиотеки pickle.

4. Распознавание изображений (отдельный скрипт):

Загрузка данных: Загружает CSV-файл (augmented_description2_extended.csv).
Загрузка модели: Загружает предварительно обученную модель из файла (feature_extraction_model.h5).
Загрузка признаков: Загружает предварительно вычисленные признаки из файла (image_features.pkl).
preprocess_new_image: Определяет функцию для предобработки новых, загружаемых пользователем изображений.
find_closest_image: Определяет функцию, которая:
	Предобрабатывает новое изображение.
	Извлекает признаки из нового изображения с помощью загруженной модели.
	Вычисляет косинусное сходство между признаками нового изображения и загруженными признаками всех изображений в датасете.
	Находит наиболее похожее изображение на основе максимального косинусного сходства.
	Возвращает название, автора и описание найденного изображения.

Пример использования:
	Задает путь к новому изображению (new_image_path).
	Вызывает функцию find_closest_image для поиска наиболее похожего изображения.
	Выводит название, автора и описание найденного изображения.


Код для обучения модели и сохранения признаков отделен от кода для распознавания изображений. 
Это позволило обучить модель и извлечь признаки только один раз, а затем использовать их для распознавания новых изображений без необходимости повторного обучения модели.
Изображение с наименьшим косинусным расстоянием (максимальным косинусным сходством) считается наиболее похожим.
Получаем ответ: название картины, автор, описание.
