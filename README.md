...
Загружены данные об изображениях и их описания из CSV-файла.(Исходный CSV-файл предварительно был расширен подробным описанием картин).
Датасет расширен аугментациями (с 1670 до 40080).
Предобработаны изображения (изменен размер, проведена нормализация значения пикселей).

Настройка модели: 
Загружена предобученная модель VGG16 (без классификационных слоев). 
Заморожены её существующие веса. 
Добавлен свой полносвязный слой сверху.

Извлечение признаков: 
Все предобработанные изображения были пропущены через модифицированную модель VGG16 для извлечения векторов признаков.

Сохранение модели: 
Модифицированная модель VGG16 (архитектура и веса) сохранена в файл .

Сохранение признаков: 
Все извлеченные векторы признаков также сохранены в файл.

Использование: 
Берем новое изображение, пропускем его через модель, чтобы получить его вектор признаков.
Во время распознавания новых изображений, вычисляются косинусные расстояния между вектором признаков нового изображения и всеми сохраненными векторами признаков. 
